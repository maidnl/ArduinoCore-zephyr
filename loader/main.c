...
#include "usbanimation.h"
	...
	if (err == 0) {
		printk("Update confirmed and made permanent!\n");
	}
#endif
...
static int dfu_flash_write(void *const priv, const uint32_t block, const uint16_t size,
	...
	printk("$$$\n");
	err = flash_area_open(data->id, &fa);
	if (err) {
		LOG_ERR("Failed to open flash area (err %d)", err);
		return err;
	}

	/* To use swap offset algorithm, the first block of the partition must be left free */
	size_t swap_offset = boot_get_image_start_offset(data->id);
	...
	if (block == 0) {
		...
		/* Erase the whole partition minus the reserved first block */
		size_t erase_size = fa->fa_size;
		LOG_INF("Erasing flash area (size 0x%08x) at offset 0x%08x", erase_size, 0);

		err = flash_area_erase(fa, 0, erase_size);
		if (err) {
	...
	uint32_t write_offset = data->downloaded;

	if (write_offset >= (fa->fa_size - swap_offset)) {
		LOG_ERR("Write offset 0x%08x exceeds partition size", write_offset);
		err = -ENOSPC;
		goto end;
	}

	uint32_t process_size = MIN(size, (fa->fa_size - swap_offset) - write_offset);
	...
	err = flash_area_write(fa, swap_offset + write_offset, buf, write_len);
	if (err) {
		LOG_ERR("Flash write failed at offset 0x%08x (len %zu)", swap_offset + write_offset,
				write_len);
		goto end;
	}
...
#define SKETCH_PARTITION_ID PARTITION_ID(slot0_partition)
...
static int dynamic_flash_write(void *const priv, const uint32_t block, const uint16_t size,
	...
	err = flash_area_open(SKETCH_PARTITION_ID, &fa);
	if (err) {
		LOG_ERR("Failed to open flash area (err %d)", err);
		return err;
	}
	...
	while (remaining_size > 0) {
		...
		/* gap region between HEADER and sketch */
